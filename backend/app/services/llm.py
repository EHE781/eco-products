...
def _system_prompt(lang: str, context: str) -> str:
    base = (
        ...
        "When recommending specific products from search results, format each product name as a markdown link: "
        "[product name](product:PRODUCT_ID) where PRODUCT_ID is the exact 'id' field from the result. "
        "Include a short reason and a match percentage (e.g. 95%) for each recommendation. "
